...
/**
 * 將 PHP 內建函數 uniqid() 產生的 ID（不含熵）直接插入到 GUID 前面
 * 並把被擠出去的原 GUID 字元刪除，維持 32 位數，並保留連字號
 */
...
/**
 * TGUID：GUID with Time
 * 將 uniqid() 產生的 ID（含熵）插入 GUID 前面，以連字號相連
 * 整串 ID 長達 14 + 8 + 32 = 54 位數，含連字號為 60 位數
 */
...
if (!function_exists('base10to62'))
{
    function base10to62($num)
    {
        ...
        $ret = '';
        do {
            $ret = base62_dict[bcmod($num, $to)] . $ret;
            $num = bcdiv($num, $to);
        } while ($num > 0);
        return $ret;
    }
...
/**
 * 將 16 進位的 GUID 轉為 62 進位
 * 轉換完畢的字串有 24 位數，含連字號有 28 位數
 */
if (!function_exists('base62_guid'))
{
    if (function_exists('base10to62'))
    {
        function base62_guid($dash = false)
        {
            // 分隔符號，預設無（連字號 = false）
            $separator = $dash ? '-' : '';
...
/**
 * 將 16 進位的 TGUID 轉為 62 進位
 * 轉換完畢的字串有 39 位數，含連字號有 45 位數
 */
if (!function_exists('base62_tguid'))
{
    if (function_exists('tguid') && function_exists('base10to62'))
    {
        function base62_tguid($dash = false)
        {
            // 分隔符號，預設無（連字號 = false）
            $separator = $dash ? '-' : '';
...
/**
 * 在 62 進位 TGUID 後面補上 3 位隨機數字，湊成 42 位
 * 生命、宇宙及萬事萬物的終極答案（Answer to the Ultimate Question of Life, The Universe, and Everything）！
 * 轉換完畢的字串有 42 位數，含連字號有 49 位數
 */
if (!function_exists('base62_tguid42'))
{
    if (function_exists('base62_tguid'))
    {
        function base62_tguid42($dash = false)
        {
            // 分隔符號，預設無（連字號 = false）
            $separator = $dash ? '-' : '';
...
/**
 * 由 62 進位 TGUID 反推其時間
 * 可檢測的最大時間為 3555-04-08 14:09:22.133048（zzzzzzzzzz）
 */
if (!function_exists('base62_guid_to_time'))
{
    function base62_guid_to_time($tguid = 0)
    {
        // 擷取前 10 位數
...
if (!function_exists('time_to_base62_guid'))
{
    function time_to_base62_guid($time = '')
    {
